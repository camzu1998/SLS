<form method="get"><br>
    ...
    <select id="zawodnik" name="zawodnik" required onchange="refreshEM();">
        <option value=""></option>
        <?php
    ...
    <span>Nr rundy:</span>
    <select name="runda" id="runda" required onchange="refreshEM();">
        <?php
            $rezultat = $polaczenie->query("SELECT * FROM `rundy` ORDER BY `ID` DESC");
            for($i=0;$i<$rezultat->num_rows;$i++){
                $wiersz = $rezultat->fetch_assoc();
    ...
    <button onclick="wyslijEM();">Wyślij</button>
</form>
